...
import { AlertTriangle, BookOpen, CheckSquare, Clock, Loader2, Info, FileDown, Trophy, Star } from 'lucide-react'; 
...
export default function AssessmentsPageTemplate() {
    ...
    if (user?.id) {
      const fetchAssessments = async () => {
        ...
        try {
          ...
          // The API now returns an object with assessments and subscription info 
  ...
  const { assessments, subscription } = pageData;
  ...
  {assessment._count.questions !== 25 ? (
    // Case 1: Assessment is misconfigured. Render a truly disabled button.
    <Button disabled className="w-full">
      Unavailable (Admin Config)
    </Button>
  ) : (
    // Case 2: Render a link styled as a button.
