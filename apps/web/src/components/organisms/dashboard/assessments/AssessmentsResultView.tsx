...
import { CheckCircle, XCircle, FileDown, Award } from 'lucide-react';
...
export function AssessmentResultsView() {
  ...
  const handleDownload = () => {
    if (submissionResult.certificate?.certificateUrl) {
      ...
      alert('Certificate PDF is not yet available.');
    }
  };
      ...
      <CardContent className="space-y-4">
        ...
        {submissionResult.isPassed && submissionResult.certificate && (
          <div className="pt-4 border-t">
            ...
            <Button className="w-full" onClick={handleDownload}><FileDown className="w-4 h-4 mr-2" />Download Certificate</Button>
          </div>
        )}
      </CardContent>
