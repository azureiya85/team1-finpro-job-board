...
export const useSubscriptionManagementStore = create<SubscriptionManagementStore>((set, get) => ({
  ...
  // Subscription List Actions
  fetchSubscriptions: async (page = 1, filters = {}) => {
    set({ loading: true, error: null });
    try {
      const response = await fetch(`/api/admin/subscription?page=${page}&limit=10&${new URLSearchParams(filters).toString()}`);
  ...
  setSubscriptionFilters: (filters) => {
    set({ filters });
    get().fetchSubscriptions(1, filters);
  },
